...
LARGE_CATUS = [pygame.image.load(f'{ASSETS}Cactus/LargeCactus1.png'),
...
def main():
    ...
    def background():                            # update, draw 
        global x_pos_bg, y_pos_bg
        image_width = BG.get_width()             # image width size (2404)
        SCREEN.blit(BG, (x_pos_bg, y_pos_bg))    # (0, 380)
        SCREEN.blit(BG, (image_width + x_pos_bg, y_pos_bg)) # (2440+0, 380)
    ...
    while run:
        ...
        if len(obstacles) == 0:
            if random.randint(0, 2) == 0:
                ...
                obstacles.append(LargeCactus(LARGE_CATUS))
